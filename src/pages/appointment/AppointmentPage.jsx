...
const AppointmentPage = () => {
  ...
  useEffect(() => {
    const fetchUserAppointments = async () => {
      if (!user) {
        console.log('No user available for fetching appointments');
        return;
      }
      ...
      // Supabase stores the UUID in the 'id' property
      ...
      try {
        ...
        console.log('Appointments received from API:', appointments);
        ...
        addNotification('Failed to load your appointments', 'error');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  // Fetch doctors data
  useEffect(() => {
    // This could be replaced with a real API call to fetch doctors from a database
    setDoctors([
      {
        id: 1,
  ...
  const handleBookAppointment = async () => {
    console.log('handleBookAppointment called with:', { 
      selectedDoctor, 
      selectedTime, 
      selectedDate,
    ...
    if (!selectedDoctor || !selectedTime || !user) {
      console.error('Missing required data for booking appointment');
      return;
    }
    
    // Log the entire user object to debug
    console.log('Current user object in booking:', user);
    
    // Supabase stores the UUID in the 'id' property
    const userId = user.id;
    
    if (!userId) {
      console.error('No user ID available for booking appointment');
      return;
    }
    ...
    try {
      ...
      // Manually fetch appointments again to ensure the list is updated
      try {
        console.log('Fetching updated appointments after booking');
      ...
      // Reset states and show confirmation
      ...
      try {
        console.log('Forcing refresh of appointments list');
        const refreshedAppointments = await appointmentApi.getUserAppointments(userId);
        
        if (refreshedAppointments && refreshedAppointments.length > 0) {
          console.log('Successfully refreshed appointments:', refreshedAppointments);
          const formattedAppointments = refreshedAppointments.map(appointment => ({
            id: appointment._id,
            doctorId: appointment.doctorId,
            doctorName: appointment.doctorName,
            doctorSpecialty: appointment.doctorSpecialty,
  ...
  const handleRescheduleSubmit = async () => {
    if (!appointmentToReschedule || !rescheduleTime) return;
    ...
    try {
      ...
      addNotification('Failed to reschedule appointment. Please try again.', 'error');
    } finally {
  ...
  const cancelAppointment = async (id) => {
    ...
    try {
      ...
      await appointmentApi.updateAppointmentStatus(id, 'cancelled');
      ...
      addNotification('Failed to cancel appointment. Please try again.', 'error');
    } finally {
      setIsLoading(false);
    }
  };
